...
__global__ void logCompresion(float* inArray, float* outArray, float c);
...
public:
	...
	static float log_c; // cnonst for log compression
	static float* d_envelop;
	static float* d_logComp;
	// init the cufft handles here
	static int NBK1, NBK2, NBK3, BKZ1, BKZ2, BKZ3; // block size and number of blocks for diff opperation
	//static dim3  BKZ;
	static cufftHandle plan;
	static cudaStream_t stream;
	static float* d_xflat, * d_ifftI, * d_ifftR;
	static cufftComplex* d_xflatComplex;
	static cufftComplex* d_fftComplex;
	static cufftComplex* d_ifftComplex;
	static cufftComplex* xflatComplex;
	static cufftComplex* fftComplex;
	static cufftComplex* ifftComplex;
	//--------------------------------------------------------

	static FILE* fp;
	static std::ofstream cudaLog;
	...
	static void onesC(float* bfHR, int pixels);
	...
	static float** cudaBackEnd::convertsingto2darray(float* imgArray, int rows, int cols);

	//void setMemmory(int);

	//static cudaDisplay* cudaDisplayHandle;

	static int initHW(bool debug = true);
	static int initSettingFile(const char* path, bool debug = true);

	static int initcudaFFT();

	static int initGPUprobeC(double*, bool debug = true);
	static int initGPUprobeL(double*, bool debug = true);

	static float** computeBModeImg();
	static float** computeBModeImgDev(bool debug = true);
	static float** computeBModeImgLinDev();
	static float** computeBModeImgLinDev2(bool debug = true);
	static float** computeBModeImgLinDiv(bool debug = true);

	static int warmUp();
	static int calculateThreads();
	static int readAndBeamForm(bool debug = true);
	static int filterBeamForm(bool debug = true);
	static int envelopAndCompress(bool debug = true);
};